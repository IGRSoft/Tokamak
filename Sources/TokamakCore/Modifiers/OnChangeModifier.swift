...
struct OnChangeModifier<V: Equatable>: ViewModifier {
  ...
  func body(content: Content) -> some View {
    // Fire initial action synchronously during body evaluation (i.e. during render).
    // FiberReconciler evaluates body() at reconcile time, so this fires within render().
    // `.onAppear` is not used because FiberReconciler does not support appearance lifecycle.
    // Note: re-fires on every re-render while `initial` is true — same trade-off as upstream TODO.
    let _ = initial ? action(value, value) : ()
    return content
