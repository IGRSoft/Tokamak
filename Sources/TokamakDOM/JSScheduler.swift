...
//
// Vendored into TokamakDOM from OpenCombineJS (JSScheduler.swift) so the package
// no longer depends on the unmaintained `OpenCombineJS` package, whose `JSPromise`
// fails to build against JavaScriptKit 0.19+ (`JSValue` no longer conforms to `Error`).
// Only `JSScheduler` is used by Tokamak's DOM renderers, and it relies solely on
// `JSDate`/`JSTimer` from JavaScriptKit + the `Scheduler` protocol from OpenCombine.
