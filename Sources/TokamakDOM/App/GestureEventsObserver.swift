...
enum GestureEventsObserver {
  // Single-threaded JS event loop (WASM): pointer events are delivered serially on the
  // main JS thread; no concurrent access to this shared publisher/closure storage.
  nonisolated(unsafe) static var publisher = CurrentValueSubject<_GesturePhase?, Never>(nil)

  nonisolated(unsafe) private static var pointerdown: JSClosure?
  nonisolated(unsafe) private static var pointermove: JSClosure?
  nonisolated(unsafe) private static var pointerup: JSClosure?
  nonisolated(unsafe) private static var pointercancel: JSClosure?
